"""Backfill real weekly net margin (this year + last year) for the Goal Tree's
"Beat last year" goal. Shopify total_revenue + Northbeam ad spend (now fixed),
Monday->Sunday weeks. Writes margin_yoy.json into the dashboard repo.

Boundaries: Shopify created_at_max is exclusive, so end = next Monday 00:00 (=Mon..Sun).
Northbeam period_ending_at is inclusive, so end = the Sunday (=Mon..Sun). Same 7-day week.
Run from the live pipeline (has creds + the fixed extractors): /usr/bin/python3 this.py
"""
import sys, os, json
...
N_WEEKS = 14
LATEST_SUNDAY = datetime(2026,5,31)
MO = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"]

def rev(mon, next_mon):            # Shopify [Mon 00:00, next-Mon 00:00) = Mon..Sun
    try: return round(float(sh.extract_orders(start_date=mon, end_date=next_mon)["summary"].get("total_revenue") or 0),2)
    except Exception as ex: print("   shopify err",mon.date(),ex); return None
def spend(mon, sun):              # Northbeam [Mon, Sun] inclusive = Mon..Sun
    try: return round(float(nb.extract_channel_performance(start_date=mon, end_date=sun).get("totals",{}).get("total_spend") or 0),2)
    except Exception as ex: print("   northbeam err",mon.date(),ex); return None
...
json.dump({"weeks":weeks,"shipPct":0.10,
           ...
           "generated_for":"2026-05-31"}, open(OUT,"w"), indent=0)
